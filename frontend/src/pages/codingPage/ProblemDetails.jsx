...
import ReactMarkdown from 'react-markdown'; // Assuming you'll add this library for better rendering

const ProblemDetails = ({ problem, mycode, setTestCode }) => {
  const { user } = useUser();
  const [activeTab, setActiveTab] = useState('Description');
  const tabs = ['Description', 'AI Tutor', 'Smart Docs', 'Submissions'];
  ...
  useEffect(() => {
    ...
    const loadSmartDocs = async () => {
        if (!problem || smartDocs.content) return;
  ...
  }, [activeTab, problem, user, smartDocs.content]);
  ...
  return (
    <div className="bg-dark-panel h-screen rounded-lg flex flex-col">
      {/* Tabs */}
      <div className="flex-shrink-0 border-b border-dark-border px-2">       {tabs.map(tab => (      <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={`px-4 cursor-pointer py-2.5 text-sm font-medium transition-colors ${             activeTab === tab
      ...
      {/* Content Pane */}
      <div className="flex-grow overflow-y-auto">   {activeTab === 'Description' && (         <div className="p-6 h-full">           <div className="flex items-center gap-4 mb-4">             <span className={`text-white text-xs font-bold px-2.5 py-1 rounded-full ${problem.difficulty === 'Easy' ? 'bg-green-500' : problem.difficulty === 'Medium' ? 'bg-yellow-500' : 'bg-red-500'}`}>{problem.difficulty.toUpperCase()}</span></div>
            ...
            {problem.examples.map((example, index) => (              <div key={index} className="bg-dark-code-editor border border-dark-border p-4 rounded-md mb-4">             <p className="font-mono text-sm"><strong className="text-text-accent">Input:</strong> {example.input}</p>
                <p className="font-mono text-sm mt-2"><strong className="text-green-400">Output:</strong> {example.output}</p>
                {example.explanation && <p className="text-sm mt-2 text-text-secondary"><strong className="text-purple-400">Explanation:</strong> {example.explanation}</p>}           </div>
            ))}
          </div>
        )}
        ...
        {activeTab === 'Smart Docs' && (
            <div className="p-6 h-full">
                ...
                {smartDocs.content && <div className="prose prose-invert max-w-none"><ReactMarkdown>{smartDocs.content}</ReactMarkdown></div>}
            </div>
        )}

        {activeTab === 'Submissions' && (
          <div className="w-full h-full">  <SubmissionsTab 
